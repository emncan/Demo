...
public class QuadTree 
{
    ...
    public QuadTree(Rect bounds, int maxBodiesPerNode = 6, int maxLevel = 6)
    {
        ...
        _maxLevel = maxLevel;
        _bodies = new List<IQuadtreeObject>(maxBodiesPerNode);
    }
    ...
    private List<IQuadtreeObject> _bodies;
    ...
    private List<IQuadtreeObject> _entCache;
    ...
    public List<IQuadtreeObject> GetBodies(Vector3 point, float radius)
    {
        var p = new Vector2(point.x, point.z);
        return GetBodies(p, radius);
    }

    public List<IQuadtreeObject> GetBodies(Vector2 point, float radius)
    {
        if (_entCache == null) _entCache = new List<IQuadtreeObject>(64);
        else _entCache.Clear();
        GetBodies(point, radius, _entCache);
        return _entCache;
    }

    public List<IQuadtreeObject> GetBodies(Rect rect)
    {
        if (_entCache == null) _entCache = new List<IQuadtreeObject>(64);
        else _entCache.Clear();
    ...
    private void GetBodies(Rect rect, List<IQuadtreeObject> bods)
    {
        //no children
        if (_childA == null)
        {
